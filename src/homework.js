...
raw_imported_data.forEach(nft => {
  ...
  const saleEvent = last_sale ? last_sale.event_timestamp : collection_created_date;
  ...
  if (reducedData.has(owner.address)) {
    ...
    reducedData.set(owner.address, { tokenId: token_id, amount: amount + 1, holdingSince: newHoldingSince.split('T', 1)[0] });
		} else {
		reducedData.set(owner.address, { tokenId: token_id, amount: 1, holdingSince: saleEvent.split('T', 1)[0]  });
  }
});
...
const top10Holder = [...sortedMap.values()].slice(TOP_10 - 1, TOP_10)[0]?.amount;
const top50Holder = [...sortedMap.values()].slice(TOP_50 - 1, TOP_50)[0]?.amount;
const top100Holder = [...sortedMap.values()].slice(TOP_100 - 1, TOP_100)[0]?.amount;
