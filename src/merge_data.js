...
async function mergeData(collectionSlug) {
  ...
  const openseaCalculatedStats = await getLastOpenseaCalculatedStats(
    collectionSlug
  );
  ...
  const lastInserted = await getConnection()
...
async function getLastCollectionStats(collectionSlug, done) {
  return await getConnection()
    .collection('collection_stats')
    .findOne({ collection_slug: collectionSlug, done }, { sort: { _id: -1 } });
}
